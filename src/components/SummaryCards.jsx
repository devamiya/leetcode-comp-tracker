  ...
  return (
    <section className="summary-cards" id="summary-cards">
      <div className="card card-stat">
        ...
        <div className="stat-value">{summary.total_posts_fetched || 0}</div>
      ...
      <div className="card card-stat">
        <div className="stat-icon">🏢</div>
        <div className="stat-value">{summary.unique_companies || 0}</div>
        <div className="stat-label">Companies</div>
      </div>
      <div className="card card-stat">
        <div className="stat-icon">💰</div>
        <div className="stat-value">{summary.overall_avg_salary != null ? formatSalary(summary.overall_avg_salary) : '—'}</div>
        <div className="stat-label">Avg Compensation</div>
      </div>
      <div className="card card-stat">
        <div className="stat-icon">📊</div>
        <div className="stat-value">{summary.overall_median_salary != null ? formatSalary(summary.overall_median_salary) : '—'}</div>
        <div className="stat-label">Median Compensation</div>
      </div>
      <div className="card card-stat">
        <div className="stat-icon">📅</div>
        <div className="stat-value" style={{ fontSize: dateRangeText.length > 20 ? '1.25rem' : '1.5rem'}}>{dateRangeText}</div>
